// Get your shorts on - this is an array workout!
    ...
    const people = [
      ...
      // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
            ...
            //get the dom elements out of the page. Find what to select
        ...
        //convert the links to a list of names. Map through the array and return the text content of each link so we can filter thru it later
        let de = links.map(link => link.textContent)
        de = de.filter(streetName => streetName.includes('de'))

        //filter the ones that include de
      ...
      // Sort the people alphabetically by last name
        let namesArray =[]
        const names = inventors.map((element) => namesArray.push(`${element.last} ${element.first}`))
    ...
    const typeOfTransportation = data.reduce(function(obj, item) {
